...
function updateDashboard(data) {
    ...
    const previousTotalItems = parseInt(totalItemsDisplay.dataset.previousItems) || 0; // Get previous total items
...
function identifyChanges(previousData, currentData) {
    ...
    currentData.forEach(currentRow => {
        const previousRow = previousData.find(row => row.rackNumber === currentRow.rackNumber);
        if (!previousRow) {
